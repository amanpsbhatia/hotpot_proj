...
//  acquire_lock(areas, 2);
  ...
  return execute_get(get_op);

 // release_lock(areas, 2);

}

int do_del_txn(struct delete_op_s* del_op) {
    if (!del_op)
	return -1;
    struct delete_op_in* p = (struct delete_op_in*)malloc(sizeof(struct delete_op_in));
    p->key = del_op->key;
    p->status = 0;
    int ret = do_del_steps(p);
    return ret;
}

int do_del_steps(struct delete_op_in* del_op) {

    if (!del_op)
	return -1;
    get_fetch_address((struct get_op_in*)del_op);
    if(del_op->status == -1 || del_op->index_pt == NULL) {
	printf("Key not found\n");
	return -1;
     }
     execute_del(del_op);
...
void acquire_lock(struct dsnvm_addr_len* areas, int n){
  ...
  printf("In acquire lock, ret is : %d\n",ret);
  perror("fail to acquire lock");
}

void release_lock(struct dsnvm_addr_len* areas, int n) {
  int ret = msync(areas, n, DSNVM_COMMIT_XACT_FLAG);
  printf("In release lock, ret is : %d\n",ret);
  perror("fail to release lock");
}
